		...
		for y := range (*board)[x] {
			...
			for i := -1; i <= 1; i++ {
				for j := -1; j <= 1; j++ {
					...
					fmt.Printf("================>analyzing neighbor [%d][%d]\n", r, c)
					...
					if (*board)[(x+i+rows)%rows][(y+j+columns)%columns].PreviousLife {
						...
						if (x-r >= -1) && (x-r <= 1) && (y-c >= -1) && (y-c <= 1) {
							...
							fmt.Printf("r (x+i+rows %% rows) => [ %d + %d ] + %d  %% %d = [%d]\n", x, i, rows, rows, r)
							fmt.Printf("c (y+j+columns %% columns) => [ %d  + %d ] + %d  %% %d =[%d]\n", y, j, columns, columns, c)
							//fmt.Printf("c (x+i+colum %% column) => %d + %d +%d %% %d\n", x, i, columns, columns)
							neighbors++
							fmt.Printf("saving color dna [%s]\n", (*board)[(x+i+rows)%rows][(y+j+columns)%columns].ColorHex)
							parentsColor = append(parentsColor, (*board)[(x+i+rows)%rows][(y+j+columns)%columns].ColorHex)
